githuPublic a As String, n, n2, pa, pb, pc, pd, pe, ty As Integer, p_no As String
...
Dim M, M2, n_products As Integer
...
n_products = WorksheetFunction.CountA(Range(c_col & c_row & ":" & c_col & myLastRow)) ' no of cells with data (no. of products /numbers)

M = CInt(InputBox("how many products?, max (all) is displayed in box", Default, n_products))

If M > n_products Or M = 0 Then
MsgBox ("You entered to many products, more than max OR 0 products please try again")
Exit Sub
End If

ReDim inputs(8, M) ' 8 x m matrix array --> m is number of products to search
ReDim inputs2(12) 'vector with generic details of sheet (product type etc)
ReDim Locs(5)
ReDim EOT_no(4, 2)
ReDim Locations(7)
...
Sheets("Locs").Select
inputs2(1) = Range("D1") 'ie "DFP6" -->add the product family type
inputs2(2) = Range("L6") 'ie "\\ukgil-ap15\diadem001$\Endurance Data"
inputs2(3) = Range("E1") 'ie  "PROJ_X"
...
inputs2(7) = Range("F1") 'Manor pod 7 naming convention for project -- ie PROJ_X
inputs2(8) = Range("G1") 'Manor pod 8 naming convention for project -- ie PROJ_X products
...
Locations(0) = inputs2(4)  'product type DFP3 or DFP6 folder on endurance data
Locations(1) = inputs2(5)  'product type DFP4 folder on endurance data
Locations(2) = Range("L7") '\\ukgil-ap15\diadem001$\Endurance Live\To Be Archived
Locations(3) = inputs2(9)  'manor pod 7 (i.e PROJ_X folder in manor pod 7)
Locations(4) = inputs2(10) 'manor pod 8 (i.e PROJ_X folder in manor pod 8)
...
inputs(0, n) = ActiveCell.Value 'product number
...
If objFolder2.Subfolders.Count > 0 Then
    For Each objSubFolder In objFolder2.Subfolders
        If InStr(1, objSubFolder, p_no, 1) > 0 And InStr(1, objSubFolder, "strip", 1) > 0 Then
            If ((objSubFolder.Attributes And 2) = 0) And ((objSubFolder.Attributes And 4) = 0) Then
            'add location to excel
                   With ActiveSheet
                     Do While Not IsEmpty(Range(a).Offset(n2 - 1, 56 + pc)) ' find and empty cell
                     pc = pc + 1
                     Loop
                        
                        prev5 = 0
                        For oldy5 = 0 To pe - 1 'check link is not same as previous
                            If objSubFolder.Name = Range(a).Offset(n2 - 1, 56 + oldy5).Hyperlinks(1).Name Then
                        ...
                        If prev5 = 0 Then
                        Range(a).Offset(n2 - 1, 56 + pc).Select
...
If ty = 0 Or ty = 1 Or ty = 2 Then
    ...
    If objFolder.Subfolders.Count > 0 Then
       For Each objSubFolder In objFolder.Subfolders
           ...
           If InStr(1, objSubFolder, p_no, 1) > 0 Then
                If ((objSubFolder.Attributes And 2) = 0) And ((objSubFolder.Attributes And 4) = 0) Then
                ReDim Preserve fold_se(p)
                fold_se(p) = objSubFolder.Path
                p = p + 1
                End If
           End If
       Next
...
Else
Set objFileSystem2 = CreateObject("Scripting.FileSystemObject")
Set objFolder2 = objFileSystem2.GetFolder(fold_se(fs))
End If
    If objFolder2.Subfolders.Count > 0 Then
        For Each objSubFolder In objFolder2.Subfolders
            If InStr(1, objSubFolder, p_no, 1) > 0 And InStr(1, objSubFolder, "charact", 1) > 0 Then
                If ((objSubFolder.Attributes And 2) = 0) And ((objSubFolder.Attributes And 4) = 0) Then
                       ...
                       'now add the file hyperlinks
                        ...
                        If Not (InStr(1, objFile.Path, "editor", 1) > 0 Or InStr(1, objFile.Path, "audit", 1) > 0 Or InStr(1, objFile.Path, "Averaged Log 10", 1) > 0 Or InStr(1, objFile.Path, "~$", 1) > 0) Then
                        f = 1 'flag to say file found
                            With ActiveSheet
                            Do While Not IsEmpty(Range(a).Offset(n2 - 1, 46 + pa)) ' find and empty cell
                            pa = pa + 1
                            Loop
                            Range(a).Offset(n2 - 1, 46 + pa).Select
                            prev2 = 0
                            For oldy2 = 0 To pa - 1 'check link is not same as previous
                                If objFile.Name = Range(a).Offset(n2 - 1, 46 + oldy2).Hyperlinks(1).Name And objSubFolder.Name = Range(a).Offset(n2 - 1, 46 + oldy2).Hyperlinks(1).SubAddress Then
                       ...
                       If f = 1 Then 'only do if folder contains a file above
                            With ActiveSheet
                  '              If objsubFolder.Name <> Range(a).Offset(n2 - 1, 58 + pd) Then
                                 Do While Not IsEmpty(Range(a).Offset(n2 - 1, 58 + pd)) ' find and empty cell
                                 pd = pd + 1
                                 Loop
                                    prev = 0
                                    For oldy = 0 To pd - 1 'check link is not same as previous
                                        If objSubFolder.Name = Range(a).Offset(n2 - 1, 58 + oldy).Hyperlinks(1).Name And Range(a).Offset(n2 - 1, 58 + oldy).Hyperlinks(1).Address = objSubFolder.Path Then
                                        pd = pd - 1 'reset the pd cell don't skip empty cell for inserting hyperlinks
                                        prev = 1
                                        End If
                                    Next
                                If prev = 0 Then
                                Range(a).Offset(n2 - 1, 58 + pd).Select
                                ActiveCell.Hyperlinks.Add Anchor:=Selection, Address:=objSubFolder.Path _
                                , TextToDisplay:=objSubFolder.Name
                                End If
                            End With
                ...
                If ((objSubFolder.Attributes And 2) = 0) And ((objSubFolder.Attributes And 4) = 0) Then
                       ...
                       'now add the file hyperlinks
                        For Each objFile In objSubFolder.Files
                            ...
                            f = 1 'flag to say file found
                            With ActiveSheet
                            Do While Not IsEmpty(Range(a).Offset(n2 - 1, 51 + pb)) ' find and empty cell
                            pb = pb + 1
                            Loop

                            prev3 = 0
                            For oldy3 = 0 To pb - 1 'check link is not same as previous
                                If objFile.Name = Range(a).Offset(n2 - 1, 51 + oldy3).Hyperlinks(1).Name And objSubFolder.Name = Range(a).Offset(n2 - 1, 51 + oldy3).Hyperlinks(1).SubAddress Then
                            ...
                            Next
                            If prev3 = 0 Then
                            Range(a).Offset(n2 - 1, 51 + pb).Select
                            ActiveCell.Hyperlinks.Add Anchor:=Selection, Address:=objFile.Path _
                            , SubAddress:=objSubFolder.Name, TextToDisplay:=objFile.Name
                            End If
                            ...
                            'ActiveCell.Offset(n2 - 1, -51 - pb).Select
                            End With
                            End If
                        Next
                        '______________________________________
                       ...
                       If f = 1 Then 'only do if folder contains a file above
                        With ActiveSheet
                          Do While Not IsEmpty(Range(a).Offset(n2 - 1, 63 + pe)) ' find and empty cell
                          pe = pe + 1
                          Loop
                            prev4 = 0
                            For oldy4 = 0 To pe - 1 'check link is not same as previous
                                If objSubFolder.Name = Range(a).Offset(n2 - 1, 63 + oldy4).Hyperlinks(1).Name And Range(a).Offset(n2 - 1, 63 + oldy4).Hyperlinks(1).Address = objSubFolder.Path Then
                                pe = pe - 1 'reset the pd cell don't skip empty cell for inserting hyperlinks
                                prev4 = 1
                                End If
                            Next
                            If prev4 = 0 Then
                            Range(a).Offset(n2 - 1, 63 + pe).Select
                            ActiveCell.Hyperlinks.Add Anchor:=Selection, Address:=objSubFolder.Path _
                            , TextToDisplay:=objSubFolder.Name
                            End If
                        End With
...
'test1 folder and map file
   ...
   ' p_no = Right(ActiveCell.Offset(0, 1), 8)
    On Error GoTo Handler:
        ...
        For Each objSubFolder In objFolder2.Subfolders
            If InStr(1, objSubFolder, p_no, 1) > 0 And InStr(1, objSubFolder, "charact", 1) > 0 Then
                If ((objSubFolder.Attributes And 2) = 0) And ((objSubFolder.Attributes And 4) = 0) Then
                       ...
                       If f = 1 Then 'only do if folder contains a file above
                            With ActiveSheet
                  '              If objsubFolder.Name <> Range(a).Offset(n - 1, 58 + pd) Then
                                 Do While Not IsEmpty(Range(a).Offset(n - 1, 58 + pd)) ' find and empty cell
                                 pd = pd + 1
                                 Loop
                                    prev = 0
                                    For oldy = 0 To pd - 1 'check link is not same as previous
                                        If objSubFolder.Name = Range(a).Offset(n - 1, 58 + oldy).Hyperlinks(1).Name And Range(a).Offset(n - 1, 58 + oldy).Hyperlinks(1).Address = objSubFolder.Path Then
                                        pd = pd - 1 'reset the pd cell don't skip empty cell for inserting hyperlinks
                                        prev = 1
                                        End If
                                    Next
        ...
        For Each objSubFolder In objFolder2.Subfolders
            If InStr(1, objSubFolder, p_no, 1) > 0 And InStr(1, objSubFolder, "burn", 1) > 0 Then
                If ((objSubFolder.Attributes And 2) = 0) And ((objSubFolder.Attributes And 4) = 0) Then
                       ...
                       If f = 1 Then 'only do if folder contains a file above
                        With ActiveSheet
                        ' If objSubFolder.Name <> Range(a).Offset(n - 1, 63 + pe) Then
                          Do While Not IsEmpty(Range(a).Offset(n - 1, 63 + pe)) ' find and empty cell
                          pe = pe + 1
                          Loop
                            prev4 = 0
                            For oldy4 = 0 To pe - 1 'check link is not same as previous
                                If objSubFolder.Name = Range(a).Offset(n - 1, 63 + oldy4).Hyperlinks(1).Name And Range(a).Offset(n - 1, 63 + oldy4).Hyperlinks(1).Address = objSubFolder.Path Then
                                pe = pe - 1 'reset the pd cell don't skip empty cell for inserting hyperlinks
                                prev4 = 1
                                End If
                            Next
